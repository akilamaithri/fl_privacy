...
from .eps_delta_edgeworth import LogLikelihoodPair, GaussianSGD
...
def get_eps(q, steps, delta, sigma, mode='moments', rdp_orders=32):
    if(mode == 'moments'):
        ...
        ## print('haha')
        sgd = GaussianSGD(sigma = sigma, p = q, order = 1)
       # eps_estimate, eps_low, eps_upper = sgd.approx_eps_from_delta_edgeworth(delta, steps)
        eps_estimate = sgd.approx_eps_from_delta_edgeworth(delta, steps)
        eps = eps_estimate
    else:
...
def get_sigma(q, T, eps, delta, init_sigma=2, interval=0.5, mode='moments'): ## init_sigma=10

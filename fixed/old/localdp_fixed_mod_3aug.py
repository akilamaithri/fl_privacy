...
class LocalDpDynamicMod:
    ...
    def __call__(
        ...
        """Performs local DP on the client model parameters."""
        
        # --- PRE-PROCESSING STEPS ---
        if msg.metadata.message_type != MessageType.TRAIN:
            return call_next(msg, ctxt)

        # Extract FitIns and its config from the message content
        ...
        # Call the next middleware in the chain to get the client's model updates
        ...
        log(
            INFO,
            "LocalDpDynamicMod: parameters are clipped by value: %.4f.",
            self.clipping_norm,
        )
        ...
        if noise_scale is not None:
            # Your dynamic noise logic
            log(INFO, "LocalDpDynamicMod: received dynamic noise scale %.4f", noise_scale)
            noise_std = noise_scale * self.clipping_norm
            ...
            log(
                INFO,
                "LocalDpDynamicMod: local DP noise added with \u03c3=%.4f",
                noise_std,
            )
        # --- FIXED NOISE FALLBACKS ---
        elif self.noise_list is not None:
            # Fixed noise from a pre-defined list
        ...
        elif self.epsilon_list is not None and self.delta_list is not None:
            # Fixed noise from epsilon/delta lists
            partition_id = ctxt.node_config["partition-id"]
            ...
            # No noise applied (non-private fallback)
            log(WARNING, "LocalDpMod: no noise parameters found, running without DP.")
